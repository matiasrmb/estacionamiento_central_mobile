...
class _AsistenciasAdminScreenState extends State<AsistenciasAdminScreen> {
  late final AsistenciasApi _api;
  ...
  void initState() {
    super.initState();
    _api = AsistenciasApi(AppServices.I.client);
    _buscar();
  }
  ...
  Future<void> _buscar() async {
    ...
    try {
      final reporte = await _api.listar(
        fechaInicio: _desde,
        fechaFin: _hasta,
      ...
      if (!mounted) return;
      setState(() => _error = 'No se pudieron cargar asistencias: $e');
    } finally {
            ...
            Row(
              children: [
                Expanded(
                  child: _SummaryCard(
                    title: 'Asistencias',
                    value: '${reporte['total_registros'] ?? 0}',
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
          ...
          for (final item in _items)
            Card(
              child: ListTile(
                ...
                title: Text(
                  '${item['usuario'] ?? ''} • ${item['activa'] == true ? 'Activo' : 'Cerrado'}',
                ),
                subtitle: Text(
                  '${_text(item['hora_inicio'])} → ${_text(item['hora_salida'] ?? 'Activo')}\n'
                  'Movimientos: ${item['cantidad_movimientos'] ?? 0} • Total: ${_money(item['total_recaudado'])}',
                ),
                isThreeLine: true,
              ),
            ),
